...
use crate::types::{Bitboard, is_any_black_castle_available, is_any_white_castle_available, Move, Mover, Piece, Position, PositionSupplement, Square, unset_bk_castle, unset_black_castles, unset_bq_castle, unset_white_castles, unset_wk_castle, unset_wq_castle};
...
pub fn make_move(position: &mut Position, mv: Move, history: &mut PositionSupplement) {
    let from = from_square_part(mv);
    let to = to_square_part(mv);
    let piece = moving_piece(position, from);
    update_history(position, history);
    if is_simple_move(history, from as Square, to, piece) {
        make_simple_move(position, mv, from as Square)
    } else {
        make_complex_move(position, mv, history)
    };
    ...
    update_supplements(position, history);
}
...
#[inline(always)]
pub fn make_complex_move(position: &mut Position, mv: Move, supplement: &PositionSupplement) {
    ...
    if promoted_piece != Empty {
        ...
        make_simple_complex_move(position, from, to, supplement);
    }
...
#[inline(always)]
pub fn make_simple_complex_move(position: &mut Position, from: Square, to: Square, supplement: &PositionSupplement) {
    ...
    position.half_moves = if test_bit(supplement.all_pieces_bitboard, to) || is_pawn_move { 0 } else { position.half_moves + 1 };
...
#[inline(always)]
pub fn is_simple_move(position: &mut PositionSupplement, from: Square, to: Square, piece: Piece) -> bool {
...
#[inline(always)]
pub fn is_simple_capture(position: &mut PositionSupplement, square: Square) -> bool {
...
pub fn default_position_history() -> PositionSupplement {
    PositionSupplement {
        history: [Position {
...
#[inline(always)]
pub fn update_history(position: &mut Position, history: &mut PositionSupplement) {
    history.history[get_move_index(position.move_number, position.mover)] = *position;
}

#[inline(always)]
pub fn update_supplements(position: &mut Position, history: &mut PositionSupplement) {
    ...
    history.white_pieces_bitboard = wpb;
    history.black_pieces_bitboard = bpb;
    history.all_pieces_bitboard = wpb | bpb;
}

#[inline(always)]
pub fn unmake_move(position: &mut Position, history: &mut PositionSupplement) {
