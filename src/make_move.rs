pub mod make_move {
    ...
    pub fn make_black_castle_move(position: &mut Position, to: Square) {
        ...
        position.all_pieces_bitboard = bpb | position.black_pieces_bitboard;
    ...
    pub fn make_move_with_promotion(position: &mut Position, mv: Move, promotion_piece: Piece) {
        ...
        let wn = create_if_promotion((promotionPiece == Knight), (position.white_pawn_bitboard), move_piece_within_bitboard(from, to, position.white_knight_bitboard), from, to);
        let bn = create_if_promotion((promotionPiece == Knight), (position.black_pawn_bitboard), move_piece_within_bitboard(from, to, position.black_knight_bitboard), from, to);
        let wb = create_if_promotion((promotionPiece == Bishop), (position.white_pawn_bitboard), move_piece_within_bitboard(from, to, position.white_bishop_bitboard), from, to);
        let bb = create_if_promotion((promotionPiece == Bishop), (position.black_pawn_bitboard), move_piece_within_bitboard(from, to, position.black_bishop_bitboard), from, to);
        let wr = create_if_promotion((promotionPiece == Rook), (position.white_pawn_bitboard), move_piece_within_bitboard(from, to, position.white_rook_bitboard), from, to);
        let br = create_if_promotion((promotionPiece == Rook), (position.black_pawn_bitboard), move_piece_within_bitboard(from, to, position.black_rook_bitboard), from, to);
        let wq = create_if_promotion((promotionPiece == Queen), (position.white_pawn_bitboard), move_piece_within_bitboard(from, to, position.white_queen_bitboard), from, to);
        let bq = create_if_promotion((promotionPiece == Queen), (position.black_pawn_bitboard), move_piece_within_bitboard(from ,to, position.black_queen_bitboard), from, to);
    ...
    pub fn switch_side(position: &Position) -> Mover {
        if position.mover == White { Black } else { White }
    }
