...
pub fn make_move(position: &mut Position, mv: Move, history: &mut PositionHistory) {
    ...
    if (white_pieces_bitboard | black_pieces_bitboard) & to_mask != 0 ||
        ...
        if promoted_piece != Empty {
            ...
            if position.mover == WHITE {
                make_simple_complex_white_move(position, from, to, white_pieces_bitboard | black_pieces_bitboard)
            } else {
                make_simple_complex_black_move(position, from, to, white_pieces_bitboard | black_pieces_bitboard)
            }
        }
...
pub fn make_white_castle_move(position: &mut Position, to: Square) {
    position.white_rook_bitboard = if to == C1_BIT {
        position.white_king_square = C1_BIT;
        clear_bit(position.white_rook_bitboard, A1_BIT) | bit(D1_BIT)
    } else {
        position.white_king_square = G1_BIT;
...
pub fn make_black_castle_move(position: &mut Position, to: Square) {
    position.black_rook_bitboard = if to == C8_BIT {
        position.black_king_square = C8_BIT;
        clear_bit(position.black_rook_bitboard, A8_BIT) | bit(D8_BIT)
    } else {
        position.black_king_square = G8_BIT;
...
#[inline(always)]
pub fn make_simple_complex_white_move(position: &mut Position, from: Square, to: Square, all_pieces: Bitboard) {
    ...
    if position.white_king_square == from {
...
#[inline(always)]
pub fn make_simple_complex_black_move(position: &mut Position, from: Square, to: Square, all_pieces: Bitboard) {
    ...
    if position.black_king_square == from { 
