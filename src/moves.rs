...
pub fn all_bits_except_friendly_pieces(position: &Position) -> Bitboard {
    !if position.main.mover == White { position.supplement.white_pieces_bitboard } else { position.supplement.black_pieces_bitboard }
}
...
pub fn generate_knight_moves(position: &Position, move_list: &mut MoveList) {
    ...
    while from_squares_bitboard != 0 {
        ...
        while to_bitboard != 0 {
            ...
            to_bitboard &= to_bitboard -1;
        }
        from_squares_bitboard &= from_square - 1;
    }
}
...
pub fn generate_slider_moves(position: &Position, piece: Piece, move_list: &mut MoveList, magic_box: &MagicBox) {
    let valid_destinations = all_bits_except_friendly_pieces(position);
    let mut from_bitboard = slider_bitboard_for_colour(position, &position.main.mover, &piece);
    while from_bitboard != 0 {
        let from_square = from_bitboard.trailing_zeros() as Square;
        let fsm = from_square_mask(from_square as Square);

        let mut to_bitboard = if piece == Bishop {
            magic_bishop(from_square as Square, magic_index_for_bishop(from_square as Square, position.supplement.all_pieces_bitboard, magic_box), magic_box)
        } else {
            magic_rook(from_square as Square, magic_index_for_rook(from_square as Square, position.supplement.all_pieces_bitboard, magic_box), magic_box)
        } & valid_destinations;
...
pub fn pawn_forward_and_capture_moves_bitboard(from_square: Square, capture_pawn_moves: &[Bitboard], non_captures: Bitboard, position: &Position) -> Bitboard {
    let eps = position.main.en_passant_square;
    let captures = if eps != EN_PASSANT_NOT_AVAILABLE && bit(eps) & en_passant_capture_rank(&position.main.mover) != 0 {
...
pub fn is_square_attacked_by(position: &Position, attacked_square: Square, mover: &Mover, magic_box: &MagicBox) -> bool {
    let all_pieces = position.supplement.all_pieces_bitboard;
    if *mover == White {
        is_square_attacked_by_any_pawn(position.main.white_pawn_bitboard, pawn_moves_capture_of_colour(Black, attacked_square)) ||
            is_square_attacked_by_any_knight(position.main.white_knight_bitboard, attacked_square) ||
            ...
            is_square_attacked_by_king(position.main.white_king_bitboard, attacked_square)
    } else {
        is_square_attacked_by_any_pawn(position.main.black_pawn_bitboard, pawn_moves_capture_of_colour(White, attacked_square)) ||
            is_square_attacked_by_any_knight(position.main.black_knight_bitboard, attacked_square) ||
            ...
            is_square_attacked_by_king(position.main.black_king_bitboard, attacked_square)
    }
}
...
pub fn rook_move_pieces_bitboard(position: &Position, mover: Mover) -> Bitboard {
    if mover == White {
        position.main.white_rook_bitboard | position.main.white_queen_bitboard
    } else {
        position.main.black_rook_bitboard | position.main.black_queen_bitboard
    }
}
...
pub fn bishop_move_pieces_bitboard(position: &Position, mover: Mover) -> Bitboard {
    if mover == White {
        position.main.white_bishop_bitboard | position.main.white_queen_bitboard
    } else {
        position.main.black_bishop_bitboard | position.main.black_queen_bitboard
    }
}
...
pub fn king_square(position: &Position, mover: Mover) -> Square {
    if mover == White {
        position.main.white_king_bitboard.trailing_zeros() as Square
    } else {
        position.main.black_king_bitboard.trailing_zeros() as Square
    }
}
