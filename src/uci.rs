...
fn cmd_uci() -> Either<String, Option<String>> {
    Right(Some(
        "id name Rusty Rival |20220410-01-Knight-Forking-Threats|\nid author Chris Moreton\noption name Clear Hash type button\nuciok"
            .parse()
            .unwrap(),
    ))
}
...
fn cmd_benchmark(uci_state: &mut UciState, search_state: &mut SearchState, parts: Vec<&str>) -> Either<String, Option<String>> {
    if parts.len() != 2 {
