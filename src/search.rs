    ...
    for iterative_depth in start_depth..=max_depth {
        ...
        loop {
            ...
            if aspire_best.1 > aspiration_window.0 && aspire_best.1 < aspiration_window.1 {
                ...
                if failed_high {
                    // NET-1190 / NET-1241: start_search returned as soon as this
                    // move beat beta; search it first on the retry.
...
pub fn start_search(position: &mut Position, legal_moves: &mut MoveScoreList, search_state: &mut SearchState, window: Window) -> PathScore {
    ...
    for mv in legal_moves {
        ...
        // NET-1190 / NET-1241: a root move that reaches the aspiration beta
        // ends this attempt. iterative_deepening widens beta and searches
        // again with this move first, so scouting the remaining moves against
        // a bound the window cannot contain would only produce results the
        // retry discards. A full window (multi-PV, or the final widening)
        // cannot fail high, so the exit is confined to single-PV aspiration
        // attempts.
