...
use rusty_rival::uci::{extract_go_param, is_legal_move, run_command};
...
pub fn it_sets_a_fen() {
    ...
    assert_eq!(run_command(&mut uci_state, &mut search_state, "position fen rnbqkbnr/pppppppp/8/8/PPPPPPPP/8/8/RNBQKBNR w KQkq - 0 1"), Right(None));
    assert_eq!(uci_state.fen.to_string(), "rnbqkbnr/pppppppp/8/8/PPPPPPPP/8/8/RNBQKBNR w KQkq - 0 1".to_string());
    assert_eq!(run_command(&mut uci_state, &mut search_state, "go perft 1"), Right(None))
}
...
pub fn it_runs_a_perft_test() {
    ...
    assert_eq!(run_command(&mut uci_state, &mut search_state, "position fen rnbqkbnr/pppppppp/8/8/PPPPPPPP/8/8/RNBQKBNR w KQkq - 0 1"), Right(None));
    assert_eq!(run_command(&mut uci_state, &mut search_state, "go perft 2"), Right(None))
}
...
pub fn it_handles_startpos() {
    ...
    assert_eq!(run_command(&mut uci_state, &mut search_state, "position fen rnbqkbnr/pppppppp/8/8/8/8/1PPPPPPP/RNBQKBNR w KQkq - 0 1"), Right(None));
    assert_ne!(uci_state.fen, START_POS);
    assert_eq!(run_command(&mut uci_state, &mut search_state, "position startpos"), Right(None));
...
pub fn it_handles_the_movelist() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "isready");
    assert_success_message(result, |message| { message.contains("readyok") });

    let result = run_command(&mut uci_state, &mut search_state, "go depth 1");
...
pub fn it_handles_a_bad_fen() {
    ...
    assert_eq!(run_command(&mut uci_state, &mut search_state, command), Left("Invalid FEN".to_string()));
}
...
pub fn it_handles_the_uci_command() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "uci");
    assert_success_message(result, |message| {
...
pub fn it_handles_the_debug_command() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "debug onn");
    assert_eq!(result, Left("usage: debug [on|off]".to_string()));
    assert_eq!(uci_state.debug, false);

    let result = run_command(&mut uci_state, &mut search_state, "debug on");
    assert_eq!(result, Right(None));
    assert_eq!(uci_state.debug, true);

    let result = run_command(&mut uci_state, &mut search_state, "debug off");
    assert_eq!(result, Right(None));
    assert_eq!(uci_state.debug, false);
...
pub fn it_handles_the_setoption_clear_hash_command() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "setoption name Clear Hash");
    assert_eq!(result, Right(None));
    match search_state.hash_table.get(&0) {
        Some(he) => panic!(),
        None => {}
...
pub fn it_handles_a_bad_setoption_name() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "setoption name asd");
...
pub fn it_handles_a_bad_setoption_cmd() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "setoption asd asd");
    assert_error_message(result, |message| {
...
pub fn it_handles_an_unknown_command() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "blah 123");
    assert_error_message(result, |message| {
...
pub fn it_handles_the_register_command() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "register all of this is ignored");
    assert_eq!(result, Right(None))
}
...
pub fn it_handles_the_ucinewgame_command() {
    ...
    let result = run_command(&mut uci_state, &mut search_state, "ucinewgame");
    assert_eq!(result, Right(None))
}
...
pub fn it_parses_params_from_a_go_command() {
    ...
    run_command(&mut uci_state, &mut search_state, "go blah 123 wtime 728 btime 182 depth 1");
...
pub fn it_extracts_a_u64_param() {
    assert_eq!(456, extract_go_param("cat", "dog 123 cat 456 fox 789"))
}