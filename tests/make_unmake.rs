...
fn test_make_unmake(fen: &str, move_str: &str, expected_fen_after: &str) {
    ...
    assert_eq!(
        ...
        "After move {}: expected {} but got {}",
        move_str, expected_fen_after, after_fen
    );
    ...
    assert_eq!(
        fen, restored_fen,
        "After unmake {}: expected {} but got {}",
        move_str, fen, restored_fen
    );

    // Also verify bitboards match
    assert_eq!(
        original.pieces[0].all_pieces_bitboard, position.pieces[0].all_pieces_bitboard,
        "White all_pieces mismatch after unmake"
    );
    assert_eq!(
        original.pieces[1].all_pieces_bitboard, position.pieces[1].all_pieces_bitboard,
        "Black all_pieces mismatch after unmake"
    );
    assert_eq!(
        original.pieces[0].pawn_bitboard, position.pieces[0].pawn_bitboard,
        "White pawn_bitboard mismatch after unmake"
    );
    assert_eq!(
        original.pieces[1].pawn_bitboard, position.pieces[1].pawn_bitboard,
