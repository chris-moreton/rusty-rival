...
fn it_returns_the_best_move_at_depth_1() {
    ...
    let mut position = get_position(&"n5k1/1P2P1n1/5q1p/P1pP4/5R2/5B2/1r2N2P/R3K1n1 w Q - 0 1".to_string());
    let mv = iterative_deepening(&mut position, 1, &mut search_state);
    assert_eq!(algebraic_move_from_move(mv), "b7a8q");

    let mut position = get_position(&"6k1/8/7p/P1pP4/4RB2/7P/1r6/4K3 w - - 0 1".to_string());
    let mv = iterative_deepening(&mut position, 1, &mut search_state);
