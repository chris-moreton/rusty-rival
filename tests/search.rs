...
fn it_returns_the_best_move_at_depth_1() {
    ...
    let mv = start_search(&position, 1, Instant::now().add(Duration::from_millis(250)), &mut search_state, &tx, vec![]);
...
fn assert_move(fen: &str, depth: u8, millis: u64, bestmove: &str) {
    ...
    let mv = start_search(&position, depth, Instant::now().add(Duration::from_millis(millis)), &mut search_state, &tx, vec![]);
...
fn it_finds_a_mate_in_6() {
    assert_move("8/8/8/1K6/4Q3/2P5/5k2/8 w - - 0 1", 11, 1000000, "b5c5,b5c4,e4g4");
}
