...
use rusty_rival::moves::{any_squares_in_bitboard_attacked, capture_moves, generate_diagonal_slider_moves, generate_straight_slider_moves, is_check, is_square_attacked, moves, quiesce_moves};
...
fn it_generates_bishop_moves_including_diagonal_queen_moves_from_a_given_fen_ignoring_checks() {
    ...
    let mut move_list = Vec::new(); generate_diagonal_slider_moves(
...
fn it_generates_rook_moves_including_horizontal_queen_moves_from_a_given_fen_ignoring_checks() {
    ...
    let mut move_list = Vec::new(); generate_straight_slider_moves(position.pieces[WHITE as usize].queen_bitboard | position.pieces[WHITE as usize].rook_bitboard, position.pieces[WHITE as usize].all_pieces_bitboard | position.pieces[BLACK as usize].all_pieces_bitboard, &mut move_list, !position.pieces[WHITE as usize].all_pieces_bitboard, 0);
    ...
    let mut move_list = Vec::new(); generate_straight_slider_moves(position.pieces[BLACK as usize].queen_bitboard | position.pieces[BLACK as usize].rook_bitboard, position.pieces[WHITE as usize].all_pieces_bitboard | position.pieces[BLACK as usize].all_pieces_bitboard, &mut move_list, !position.pieces[BLACK as usize].all_pieces_bitboard, 0);
    let mut algebraic: Vec<String> = move_list.iter().map(|m| { algebraic_move_from_move(*m) }).collect();
    algebraic.sort();
...
pub fn it_gets_all_capture_moves_for_a_position() {
    assert_eq!(sort_moves(quiesce_moves(&get_position(&"n5k1/1P2P1n1/1n2q2p/P1pP4/5R2/3K1B2/1r2N2P/6r1 w - c6 0 1".to_string()))),
