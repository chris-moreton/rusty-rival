...
fn it_gets_the_material_score() {
    ...
    assert_eq!(material_score(&position), -PAWN_VALUE_AVERAGE - 37);

    let position = get_position("r3k3/1R5p/8/8/p1PQ4/7P/4p3/6K1 b q - 0 1");
    assert_eq!(material_score(&position), QUEEN_VALUE_AVERAGE - PAWN_VALUE_AVERAGE + 35);
}
...
fn test_knight_blockade_penalty_position_9() {
    ...
    assert!(eval.abs() < 400, "Position 9 should evaluate near 0 (drawn), got {}", eval);
}
